...
private enum GardenLayout {
    ...
    static let atlasSceneHeightRatio: CGFloat = 0.60
    static let atlasSceneMinHeight: CGFloat = 520
    static let atlasSceneMaxHeight: CGFloat = 620
}

private enum TodayMainArtwork {
    static let sourceSize = CGSize(width: 1290, height: 2796)
    static let potFrameInSource = CGRect(x: 312, y: 827, width: 661, height: 535)
}
...
struct RootView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            if let tab = activeTab {
                BottomNav(active: tab, showsAtlasGlow: store.hasUnseenAtlasBloom) { selected in
            ...
            if store.screen == .today, let bloom = store.recentBloom {
                BloomCollectOverlay(bloom: bloom)
                    .transition(.opacity.combined(with: .move(edge: .bottom)))
                    .zIndex(2)
            }
    ...
    private var activeTab: MainTab? {
        ...
        case .us:
            .us
        case .onboarding, .health, .flowerDetail:
            nil
        }
    }
}
...
struct TodayView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ZStack(alignment: .top) {
                TodayMainBackground(size: proxy.size)
                ...
                TodayMainPotOverlay(size: proxy.size)
    ...
    @ViewBuilder
    private var todayPlantArtwork: some View {
        PlantView(
...
private struct TodayMainBackground: View {
    let size: CGSize

    var body: some View {
        Image("TodayMainBackgroundBase")
            .resizable()
            .scaledToFill()
...
private struct TodayMainPotOverlay: View {
    let size: CGSize

    private var filledArtworkFrame: CGRect {
        let scale = max(
            size.width / TodayMainArtwork.sourceSize.width,
            size.height / TodayMainArtwork.sourceSize.height
        )
        let scaledSize = CGSize(
            width: TodayMainArtwork.sourceSize.width * scale,
            height: TodayMainArtwork.sourceSize.height * scale
        )
        return CGRect(
    ...
    private var potFrame: CGRect {
        let artworkFrame = filledArtworkFrame
        let scale = artworkFrame.width / TodayMainArtwork.sourceSize.width
        let sourceFrame = TodayMainArtwork.potFrameInSource
        return CGRect(
            x: artworkFrame.minX + sourceFrame.minX * scale,
            y: artworkFrame.minY + sourceFrame.minY * scale,
    ...
    var body: some View {
        Image("TodayMainPot")
            ...
            .position(x: potFrame.midX, y: potFrame.midY)
            .frame(width: size.width, height: size.height)
            .ignoresSafeArea()
    }
}

private struct AtlasTopScene: View {
    let size: CGSize
    let garden: Garden
    let locked: Bool
    let height: CGFloat
    ...
    private var bottomFadeHeight: CGFloat {
        min(max(height * 0.12, 56), 76)
    }

    var body: some View {
        ZStack(alignment: .bottom) {
            Image(garden.atlasSceneAssetName)
                .resizable()
                .scaledToFill()
            ...
            VStack(spacing: 0) {
                ...
                LinearGradient(
                    stops: [
                        ...
                        .init(color: GardenTheme.paper.opacity(0.18), location: 0.48),
                        .init(color: GardenTheme.paper.opacity(0.76), location: 0.82),
                        .init(color: GardenTheme.paper, location: 1.0)
                    ],
                    startPoint: .top,
                    endPoint: .bottom
                )
                .frame(width: size.width, height: bottomFadeHeight)
            }
            .frame(width: size.width, height: height)
...
struct AtlasView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            let sceneHeight = atlasSceneHeight(for: proxy.size)
            ...
            ZStack(alignment: .top) {
                ...
                AtlasTopScene(
                    size: proxy.size,
                    garden: store.atlasGarden,
                    locked: !store.isGardenUnlocked(store.progress.atlasGardenIndex),
                    height: sceneHeight
                )
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        Color.clear
                            .frame(height: atlasSceneSpacerHeight(for: sceneHeight))
                        ...
                        ZStack {
                            WhiteCard {
                                VStack(alignment: .leading, spacing: 14) {
                                    ...
                                    Spacer(minLength: 8)
                                    let isUnlocked = store.isGardenUnlocked(store.progress.atlasGardenIndex)
                                    PrimaryButton(
                                        title: isUnlocked ? "查看花园" : "待解锁",
                                        systemImage: isUnlocked ? nil : "lock.fill",
    ...
    private func atlasSceneHeight(for size: CGSize) -> CGFloat {
        min(
            max(size.height * GardenLayout.atlasSceneHeightRatio, GardenLayout.atlasSceneMinHeight),
            GardenLayout.atlasSceneMaxHeight
        )
    }

    private func atlasSceneSpacerHeight(for sceneHeight: CGFloat) -> CGFloat {
        max(sceneHeight - 130, 360)
    }
...
private struct FlowerDetailSheet: View {
    ...
    var showsBloomCompletionStatus = false
    let onClose: () -> Void
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 22) {
                    ...
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        Text(mystery ? "绽放形态暂时隐藏，等真正收集时再揭晓。" : summaryText)
                            .font(.body.weight(.medium))
                            .foregroundStyle(GardenTheme.muted)
                            .lineSpacing(4)
                            .multilineTextAlignment(showsBloomCompletionStatus ? .center : .leading)
                            .frame(maxWidth: .infinity, alignment: showsBloomCompletionStatus ? .center : .leading)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                    .padding(.horizontal, 28)
                    .padding(.bottom, 30)
            ...
            if let bottomButtonTitle {
                PrimaryButton(title: bottomButtonTitle) {
                    onClose()
                }
                .padding(.horizontal, 28)
                .padding(.top, 4)
    ...
    private var summaryText: String {
        showsBloomCompletionStatus ? "花朵绽放，已加入花园收藏。" : stage.summary
    }
}

private struct BloomCollectOverlay: View {
    @EnvironmentObject private var store: GardenStore
    let bloom: BloomSnapshot

    var body: some View {
        let garden = store.gardens.first { $0.id == bloom.gardenId } ?? store.currentGarden
        let stage = store.selectedPreviewStage ?? .bloom
        ...
        GeometryReader { proxy in
            ZStack(alignment: .bottom) {
                Color.black.opacity(0.26)
                    .ignoresSafeArea()
                    .onTapGesture {
                        store.dismissRecentBloom()
                    }

                FlowerDetailSheet(
                    garden: garden,
                    index: bloom.flowerIndex,
                    stage: stage,
                    collected: true,
                    isCurrent: false,
                    mystery: false,
                    topButtonTitle: nil,
                    bottomButtonTitle: "返回",
                    showsBloomCompletionStatus: true
                ) {
                    store.dismissRecentBloom()
                }
                .frame(maxWidth: .infinity)
                .frame(height: max(560, proxy.size.height - proxy.safeAreaInsets.top - proxy.safeAreaInsets.bottom - 40))
                .padding(.horizontal, 12)
                .padding(.vertical, 20)
            }
        }
